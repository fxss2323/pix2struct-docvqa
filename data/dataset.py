...
# 使用配置文件中的路径
IMDB = os.path.join(project_root, config.IMDB)
IMG = os.path.join(project_root, config.IMG)
MODEL_PATH = os.path.join(project_root, config.MODEL_NAME) if not config.MODEL_NAME.startswith('google/') else config.MODEL_NAME
FONT_PATH = os.path.join(project_root, config.FONT_PATH)
...
class MPDocVQA(Dataset):
    def __init__(self, imdb_dir, img_dir, split):
        ...
        self.header = data[0]
        if split == 'train':
            self.imdb = data[1:501]
        elif split == 'val':
            self.imdb = data[1:101]
        else:
            self.imdb = data[1:]
        #self.imdb = data[1:]
    ...
    def __getitem__(self, idx):
        ...
        for img_name in final_image_names:
            ...
            inputs = self.processor(images=[image], text=[question], return_tensors='np', font_path=FONT_PATH, padding=True)
